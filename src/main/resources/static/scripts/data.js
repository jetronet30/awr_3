...
export function initDataModule() {
    const netContainer = document.querySelector("#data-container");
    ...
    /** --- ⚙️ data set --- */
    const form = netContainer.querySelector("#data-set-from");
    if (!form) return;
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (!result.success && result.message) {
                alert("DATA პარამეტრების განახლება ჩაიშალა: " + result.message);
            } else if (!result.success) {
                alert("DATA პარამეტრების განახლება ჩაიშალა: უცნობი შეცდომა");
            }
        } catch (err) {
            content.innerHTML = `<p style="color:red;">Error applying DATA settings.</p>`;
            updateIndicator(false);
            alert("შეცდომა DATA პარამეტრების განახლებისას: " + err.message);
        }
    };

    // ორივე ღილაკს ვუკავშირებთ ერთსა და იმავე handler-ს
    const setBtn = form.querySelector("#data-set-btn");
    const cloneBtn = netContainer.querySelector("#data-set-btn-clone");
    ...
    /** --- 🌐 write reboot ფორმა --- */
    const setWRForm = netContainer.querySelector('form[action="/data-save-and-reboot"]');
    const setWRBtn = setWRForm?.querySelector("#data-set-and-reboot-btn");

    if (setWRForm && setWRBtn) {
        setWRBtn.replaceWith(setWRBtn.cloneNode(true));
        const newSetWRBtn = netContainer.querySelector("#data-set-and-reboot-btn");
        ...
        const handler = async (e) => {
            ...
            try {
                ...
                if (content.querySelector("#network-container")) {
                    const mod = await import(`/scripts/data.js?v=${Date.now()}`);
                    mod.initDataModule();
                }
...
export function cleanupDataModule() {
    intervalIds.forEach(id => clearTimeout(id));
    intervalIds.clear();
    clearEventListeners();
    console.log('✅ Data module cleaned up');
}
