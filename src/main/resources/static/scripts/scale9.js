// =============================================================================
// scale9.js — მოდული #scale9-container-ისთვის (განახლებული 2025/2026)
// =============================================================================

let intervalIds = new Set();
let eventListeners = new Map();
let hlsInstanceScale9 = null;
let videoObserver = null;
let weightEventSource = null;

// === ინიციალიზაციის ფლაგი (მხოლოდ ერთხელ) ===
let isScale9Initialized = false;
...
function getAllowedWagonLength(netContainer) {
    const input = netContainer.querySelector("#magonNumLeght_9");
...
// =============================================================================
// Helper: conId_9-ის მიღება
// =============================================================================
function getConId(netContainer) {
    const input = netContainer.querySelector("#conId_9");
    return input?.value?.trim() || "unknown";
}

// =============================================================================
// Helper: კავშირის ინდიკატორის განახლება (#con9-indicator)
// =============================================================================
function updateConIndicator(success) {
    const indicator = document.querySelector("#con9-indicator");
    if (!indicator) return;
...
// =============================================================================
// INITIAL LOAD: /showweighingWagons9 → #operation-data9-container
// =============================================================================
async function loadInitialWagonData(netContainer, updateIndicator) {
    if (!netContainer) return;

    const targetContainer = netContainer.querySelector("#operation-data9-container");
    if (!targetContainer) return;

    try {
        const response = await fetch("/showweighingWagons9", { method: "POST" });
...
// =============================================================================
// BIND EDIT FORMS: .oprdata9-set-from ფორმები + ავტომატური გაგზავნა
// =============================================================================
function bindEditWagonForm(netContainer, updateIndicator) {
    const editForms = netContainer.querySelectorAll('form.oprdata9-set-from');
    const allowedLength = getAllowedWagonLength(netContainer);

    editForms.forEach((editForm) => {
        const editBtn = editForm.querySelector('.oprdata9-set-btn');
        const wagonNumberInput = editForm.querySelector('.scale9-operdata-wagonNum-input');
...
// ================================================
function initScale9PrintButton() {
    const printBtn = document.getElementById("scale9-print-btn");
    if (!printBtn) return;
    ...
    cleanBtn.addEventListener("click", () => {
        ...
        iframe.onerror = () => {
            alert("PDF ფაილის ჩატვირთვა ვერ მოხერხდა (/pdf9).");
...
// =============================================================================
function initVideoScale9() {
    const video = document.getElementById('player-9');
    if (!video || !video.getAttribute('data-hls-src')) return;

    if (hlsInstanceScale9) {
        hlsInstanceScale9.destroy();
        hlsInstanceScale9 = null;
    }

    if (Hls.isSupported()) {
        hlsInstanceScale9 = new Hls({
            maxBufferLength: 15,
        ...
        hlsInstanceScale9.loadSource(video.getAttribute('data-hls-src'));
        hlsInstanceScale9.attachMedia(video);

        hlsInstanceScale9.on(Hls.Events.MANIFEST_PARSED, () => video.play().catch(() => {}));
        hlsInstanceScale9.on(Hls.Events.ERROR, (event, data) => {
            if (data.fatal || data.details === 'levelLoadError') {
                hlsInstanceScale9.destroy();
                hlsInstanceScale9 = null;
                setTimeout(initVideoScale9, 2000);
            }
        });
...
function observeVideoScale9(container) {
    if (videoObserver) videoObserver.disconnect();

    videoObserver = new MutationObserver(() => {
        const video = document.getElementById('player-9');
        if (video && !hlsInstanceScale9 && video.getAttribute('data-hls-src')) {
            initVideoScale9();
        }
    });
...
function connectWeightSSE(netContainer, updateIndicator) {
    ...
    weightEventSource = new EventSource('/sendscale9');
    ...
    weightEventSource.addEventListener(getConId(netContainer), (e) => {
        ...
        switch (data) {
            ...
            case 'update-data-works-start':
                document.getElementById('w-indic-9').value = "START";
                document.getElementById('w-indic-9').style.color = '#fdec04ff';
                break;

            case 'update-data-works-stop':
                document.getElementById('w-indic-9').value = "END";
                document.getElementById('w-indic-9').style.color = '#fd048dff';
                break;
            ...
            default:
                // წონის მნიშვნელობა
                const weightInput = document.getElementById('w-indic-9');
                if (weightInput) {
    ...
    weightEventSource.onopen = () => {
        console.log('SSE connected: /sendscale9');
        updateConIndicator(true);
    };
...
// =============================================================================
export function initScale9Module() {
    if (isScale9Initialized) {
        console.warn("scale9 უკვე ინიციალიზებულია. გამოტოვება.");
        return;
    }

    isScale9Initialized = true;
    const netContainer = document.querySelector("#scale9-container");
    const content = document.querySelector("main.content");

    if (!netContainer || !content) {
        isScale9Initialized = false;
        return;
    }

    function updateIndicator(success) {
        const indicator = document.querySelector("#scale9-indicator");
        if (indicator) {
            indicator.style.transition = "background-color 0.5s ease";
            indicator.style.backgroundColor = success ? "#00cc66" : "#ff3333";
    ...
    const reloadModule = async (actionUrl) => {
        ...
        try {
            ...
            if (content.querySelector("#scale9-container")) {
                const mod = await import(`/scripts/scale9.js?v=${Date.now()}`);
                mod.initScale9Module();
            }
            updateIndicator(true);
        } catch (err) {
    ...
    // 3. START WEIGHING
    const startForm = netContainer.querySelector('form[action="/startWeighing_9"]' );
    const startBtn = startForm?.querySelector("#scale9-start-weighing-btn");
    ...
    // 4. DONE WEIGHING
    const doneForm = netContainer.querySelector('form[action="/doneWeighing_9"]');
    const doneBtn = doneForm?.querySelector("#scale9-done-weighing-btn");
    if (doneForm && doneBtn) {
    ...
    // 5. ABORT WEIGHING
    const abortForm = netContainer.querySelector('form[action="/abortWeighing_9"]');
    const abortBtn = abortForm?.querySelector("#scale9-abort-weighing-btn");
    ...
    // 6. ADD WAGON
    const addForm = netContainer.querySelector('form[action="/addwagonWeighing_9"]');
    const addBtn = addForm?.querySelector("#scale9-add-wagon-btn");
    const wagonNumberInput = addForm?.querySelector("#scale9-number-input");
    ...
    if (addForm && addBtn && wagonNumberInput) {
        ...
        const submitHandler = async (e) => {
            ...
            try {
                ...
                ['#scale9-number-input', '#scale9-product-input', '#scale9-count-input'].forEach(sel => {
    ...
    // 7. UPDATE ALL
    const updateForm = netContainer.querySelector('form[action="/updateAllWeighing_9"]');
    const updateBtn = updateForm?.querySelector("#scale9-update-weighing-btn");
    ...
    // 8. VIDEO
    initVideoScale9();
    observeVideoScale9(netContainer);

    // 9. Global cleanup access
    window.cleanupScale9Module = cleanupScale9Module;
}

// =============================================================================
// FULL CLEANUP
// =============================================================================
export function cleanupScale9Module() {
    console.log('Cleaning up scale9 module...');
    ...
    const indicators = ["#scale9-indicator", "#con9-indicator"];
    ...
    console.log('scale9 module fully cleaned');
}

// Auto-cleanup on page unload
window.addEventListener('beforeunload', () => {
    if (typeof cleanupScale9Module === 'function') {
        cleanupScale9Module();
    }
});