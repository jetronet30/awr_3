// =============================================================================
// scale1.js — მოდული #scale1-container-ისთვის (განახლებული 2025/2026)
// =============================================================================
...
// =============================================================================
// Helper: conId_1-ის მიღება
// =============================================================================
function getConId(netContainer) {
    const input = netContainer.querySelector("#conId_1");
    return input?.value?.trim() || "unknown";
}

// =============================================================================
// Helper: კავშირის ინდიკატორის განახლება (#con1-indicator)
// =============================================================================
function updateConIndicator(success) {
    const indicator = document.querySelector("#con1-indicator");
    if (!indicator) return;
...
// ================================================
function initScale1PrintButton() {
    const printBtn = document.getElementById("scale1-print-btn");
    if (!printBtn) return;
    ...
    weightEventSource.addEventListener(getConId(netContainer), (e) => {
        ...
        switch (data) {
            ...
            case 'update-data-works-start':
                document.getElementById('w-indic-1').value = "START";
                document.getElementById('w-indic-1').style.color = '#fdec04ff';
                break;

            case 'update-data-works-stop':
                document.getElementById('w-indic-1').value = "END";
                document.getElementById('w-indic-1').style.color = '#fd048dff';
                break;
...
export function initScale1Module() {
    ...
    // 4. DONE WEIGHING
    const doneForm = netContainer.querySelector('form[action="/doneWeighing_1"]');
    const doneBtn = doneForm?.querySelector("#scale1-done-weighing-btn");
    if (doneForm && doneBtn) {
...
export function cleanupScale1Module() {
    ...
    const indicators = ["#scale1-indicator", "#con1-indicator"];
