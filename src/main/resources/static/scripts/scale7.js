// =============================================================================
// scale7.js — მოდული #scale7-container-ისთვის (განახლებული 2025/2026)
// =============================================================================

let intervalIds = new Set();
let eventListeners = new Map();
let hlsInstanceScale7 = null;
let videoObserver = null;
let weightEventSource = null;

// === ინიციალიზაციის ფლაგი (მხოლოდ ერთხელ) ===
let isScale7Initialized = false;
...
function getAllowedWagonLength(netContainer) {
    const input = netContainer.querySelector("#magonNumLeght_7");
...
// =============================================================================
// Helper: conId_7-ის მიღება
// =============================================================================
function getConId(netContainer) {
    const input = netContainer.querySelector("#conId_7");
    return input?.value?.trim() || "unknown";
}

// =============================================================================
// Helper: კავშირის ინდიკატორის განახლება (#con7-indicator)
// =============================================================================
function updateConIndicator(success) {
    const indicator = document.querySelector("#con7-indicator");
    if (!indicator) return;
...
// =============================================================================
// INITIAL LOAD: /showweighingWagons7 → #operation-data7-container
// =============================================================================
async function loadInitialWagonData(netContainer, updateIndicator) {
    if (!netContainer) return;

    const targetContainer = netContainer.querySelector("#operation-data7-container");
    if (!targetContainer) return;

    try {
        const response = await fetch("/showweighingWagons7", { method: "POST" });
...
// =============================================================================
// BIND EDIT FORMS: .oprdata7-set-from ფორმები + ავტომატური გაგზავნა
// =============================================================================
function bindEditWagonForm(netContainer, updateIndicator) {
    const editForms = netContainer.querySelectorAll('form.oprdata7-set-from');
    const allowedLength = getAllowedWagonLength(netContainer);

    editForms.forEach((editForm) => {
        const editBtn = editForm.querySelector('.oprdata7-set-btn');
        const wagonNumberInput = editForm.querySelector('.scale7-operdata-wagonNum-input');
...
// =============================================================================
function initVideoScale7() {
    const video = document.getElementById('player-7');
    if (!video || !video.getAttribute('data-hls-src')) return;

    if (hlsInstanceScale7) {
        hlsInstanceScale7.destroy();
        hlsInstanceScale7 = null;
    }
    ...
    if (Hls.isSupported()) {
        ...
        hlsInstanceScale7.attachMedia(video);

        hlsInstanceScale7.on(Hls.Events.MANIFEST_PARSED, () => video.play().catch(() => {}));
        hlsInstanceScale7.on(Hls.Events.ERROR, (event, data) => {
            if (data.fatal || data.details === 'levelLoadError') {
                hlsInstanceScale7.destroy();
                hlsInstanceScale7 = null;
                setTimeout(initVideoScale7, 2000);
            }
        });
...
function observeVideoScale7(container) {
    if (videoObserver) videoObserver.disconnect();

    videoObserver = new MutationObserver(() => {
        const video = document.getElementById('player-7');
        if (video && !hlsInstanceScale7 && video.getAttribute('data-hls-src')) {
            initVideoScale7();
        }
    });
...
function connectWeightSSE(netContainer, updateIndicator) {
    ...
    weightEventSource = new EventSource('/sendscale7');
    ...
    weightEventSource.addEventListener(getConId(netContainer), (e) => {
        ...
        switch (data) {
            ...
            case 'update-data-works-start':
                document.getElementById('w-indic-7').value = "START";
                document.getElementById('w-indic-7').style.color = '#fdec04ff';
                break;

            case 'update-data-works-stop':
                document.getElementById('w-indic-7').value = "END";
                document.getElementById('w-indic-7').style.color = '#fd048dff';
                break;
            ...
            default:
                // წონის მნიშვნელობა
                const weightInput = document.getElementById('w-indic-7');
                if (weightInput) {
    ...
    weightEventSource.onopen = () => {
        console.log('SSE connected: /sendscale7');
        updateConIndicator(true);
    };
...
// =============================================================================
export function initScale7Module() {
    if (isScale7Initialized) {
        console.warn("scale7 უკვე ინიციალიზებულია. გამოტოვება.");
        return;
    }

    isScale7Initialized = true;
    const netContainer = document.querySelector("#scale7-container");
    const content = document.querySelector("main.content");

    if (!netContainer || !content) {
        isScale7Initialized = false;
        return;
    }

    function updateIndicator(success) {
        const indicator = document.querySelector("#scale7-indicator");
        if (indicator) {
            indicator.style.transition = "background-color 0.5s ease";
            indicator.style.backgroundColor = success ? "#00cc66" : "#ff3333";
    ...
    const reloadModule = async (actionUrl) => {
        ...
        try {
            ...
            if (content.querySelector("#scale7-container")) {
                const mod = await import(`/scripts/scale7.js?v=${Date.now()}`);
                mod.initScale7Module();
            }
            updateIndicator(true);
        } catch (err) {
    ...
    // 3. START WEIGHING
    const startForm = netContainer.querySelector('form[action="/startWeighing_7"]' );
    const startBtn = startForm?.querySelector("#scale7-start-weighing-btn");
    ...
    // 4. DONE WEIGHING
    const doneForm = netContainer.querySelector('form[action="/doneWeighing_7"]');
    const doneBtn = doneForm?.querySelector("#scale7-done-weighing-btn");
    if (doneForm && doneBtn) {
    ...
    // 5. ABORT WEIGHING
    const abortForm = netContainer.querySelector('form[action="/abortWeighing_7"]');
    const abortBtn = abortForm?.querySelector("#scale7-abort-weighing-btn");
    ...
    // 6. ADD WAGON
    const addForm = netContainer.querySelector('form[action="/addwagonWeighing_7"]');
    const addBtn = addForm?.querySelector("#scale7-add-wagon-btn");
    const wagonNumberInput = addForm?.querySelector("#scale7-number-input");
    ...
    if (addForm && addBtn && wagonNumberInput) {
        ...
        const submitHandler = async (e) => {
            ...
            try {
                ...
                ['#scale7-number-input', '#scale7-product-input', '#scale7-count-input'].forEach(sel => {
    ...
    // 7. UPDATE ALL
    const updateForm = netContainer.querySelector('form[action="/updateAllWeighing_7"]');
    const updateBtn = updateForm?.querySelector("#scale7-update-weighing-btn");
...
// FULL CLEANUP
// =============================================================================
export function cleanupScale7Module() {
    console.log('Cleaning up scale7 module...');
    ...
    const indicators = ["#scale7-indicator", "#con7-indicator"];
    ...
    console.log('scale7 module fully cleaned');
}

// Auto-cleanup on page unload
window.addEventListener('beforeunload', () => {
    if (typeof cleanupScale7Module === 'function') {
        cleanupScale7Module();
    }
});