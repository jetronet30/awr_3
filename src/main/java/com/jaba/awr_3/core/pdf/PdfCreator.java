...
public class PdfCreator {
    ...
    // New constants for title and logo
    private static final String TITLE = "Port of Poti";
    private static final String LOGO_PATH = RepoInit.LOGO_REPO.getAbsolutePath()+"/logo.png"; // Replace with the actual path to your logo file (e.g., PNG or JPEG)
    private static final float TITLE_FONT_SIZE = 24f;
    private static final float LOGO_WIDTH = 50f; // Adjust logo width as needed (height will scale proportionally)
    private static final float SPACE_AFTER_TITLE = 20f; // Space between title and logo
    private static final float SPACE_AFTER_LOGO = 30f; // Space between logo and main content

    // FontManager will handle all fonts
    private FontManager fontManager;

    public void createPdfWeb(TrainMod train) {
        if (train == null || train.getWagons() == null || train.getWagons().isEmpty()) return;
        ...
        try (PDDocument doc = new PDDocument()) {
            ...
            // Draw Process ID (top right, as before)
            drawProcessId(cs, fontManager.getLatinFont(), page, processId);

            // Draw centered title below Process ID
            float pageHeight = page.getMediaBox().getHeight();
            float pageWidth = page.getMediaBox().getWidth();
            PDType0Font titleFont = fontManager.getLatinFont();
            float titleWidth = (titleFont.getStringWidth(TITLE) / 1000f) * TITLE_FONT_SIZE;
            float titleX = (pageWidth - titleWidth) / 2;
            float titleY = pageHeight - 60; // Position below Process ID (adjust if needed)
            cs.beginText();
            ...
            // Draw centered logo below title
            PDImageXObject logo = PDImageXObject.createFromFile(LOGO_PATH, doc);
            float logoHeight = logo.getHeight() * (LOGO_WIDTH / logo.getWidth());
            float logoX = (pageWidth - LOGO_WIDTH) / 2;
            float logoY = titleY - TITLE_FONT_SIZE - SPACE_AFTER_TITLE; // Position below title (baseline adjustment)
            cs.drawImage(logo, logoX, logoY, LOGO_WIDTH, logoHeight);

            // Adjust starting y for main content below logo
            float y = logoY - logoHeight - SPACE_AFTER_LOGO;
    ...
    private void flushRun(PDPageContentStream cs, StringBuilder run, PDType0Font font, float fontSize) throws IOException {
    ...
    private void drawProcessId(PDPageContentStream cs, PDType0Font font, PDPage page, String processId) throws IOException {
        float pageWidth = page.getMediaBox().getWidth();
    ...
    private EnsureResult ensureSpace(PDDocument doc, PDPageContentStream cs, float y, float neededSpace) throws IOException {
