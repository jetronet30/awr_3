...
public class ProcesTcp1 {
    ...
    private int port;
    private final int scaleIndex = 6; // TcpService-ში პირველი TCP კონფიგურაციის ინდექსი (მუდმივი 6-ით)

    private static final Logger log = LoggerFactory.getLogger(ProcesTcp1.class);
                    ...
                    if (packet.length >= 3 && packet[0] == 0x02) {
                        try {
                            ...
                            if (CEnd240A.contains("CEnd240A")) {
                                sendDataTSR4000(GlobalRight.getREOTD_6());
                                log.info("CEnd240A: " + GlobalRight.getREOTD_6());
                            }
                        } catch (Exception e) {
